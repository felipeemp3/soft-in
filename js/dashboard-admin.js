...
const programasPorRol = {
  ...
  enfermera: ["Auxiliar de Enfermería", "Técnico en Enfermería", "Enfermería Profesional"],
  bienestar: ["Psicología", "Trabajo Social", "Terapia Ocupacional"],
  vigilante: ["Seguridad y Vigilancia", "Técnico en Seguridad"],
  administrativo: ["Administración", "Sistemas", "Contabilidad", "Recursos Humanos"],
}

// Verificar acceso de administrador al cargar la página
document.addEventListener("DOMContentLoaded", () => {
  // Verificar que solo administradores puedan acceder
  if (!window.checkRoleAccess("administrativo")) {
    return
  }
...
function loadUsers() {
  // Obtener usuarios de la base de datos simulada
  allUsers = window.database.getAllPersonas()
    // Filtrar solo usuarios válidos (con rol y nombre)
    .filter(
      (u) =>
        u &&
        typeof u === "object" &&
...
function renderUsersTable() {
  ...
  filteredUsers.forEach((user) => {
    ...
    row.innerHTML = `
            ...
            <td><span class="role-badge ${user.rol}">${user.rol.toUpperCase()}</span></td>
            <td>${user.programa_formacion || "N/A"}</td>
            <td><span class="status-badge ${user.estado_formacion.toLowerCase().replace(" ", "-")}">${user.estado_formacion}</span></td>
            <td>
                <div class="action-buttons">
                    <button class="btn-action btn-edit" onclick="editUser(${user.id_persona})">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn-action btn-delete" onclick="deleteUser(${user.id_persona})">
