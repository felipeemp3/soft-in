...
function loadUserData() {
  ...
  if (currentUser.especialidad) {
    ...
    specialtyElement.style.cssText = `
      font-size: 14px;
      color: #7f8c8d;
      margin-top: 5px;
      font-weight: 500;
    `
    specialtyElement.textContent = `Especialidad: ${currentUser.especialidad}`
    document.querySelector(".welcome-section").appendChild(specialtyElement)
  }
}
...
function updateNursingStats() {
  const stats = {
    ...
    medications: Math.floor(Math.random() * 30) + 15,
    assessments: Math.floor(Math.random() * 20) + 8,
    hours: (Math.random() * 4 + 4).toFixed(1),
  }
